...
def xu_ly_du_lieu():
    ...
    if ho_ten == "" or mssv == "":
        ...
        nhan_ket_qua.config(text="Hệ thống sẵn sàng", fg="black")
        return
    
    # Kiểm tra MSSV là số
    if not mssv.isdigit():
        messagebox.showwarning("Chú ý", "MSSV phải là số!")
        nhan_ket_qua.config(text="Hệ thống sẵn sàng", fg="black")
        return
...
nut_xac_nhan.grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
